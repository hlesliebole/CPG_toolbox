...
MopNums(MopNums < 1340)=[];
MopNums=2:930;
for MopID=MopNums
    
%--------

% first just read in the smaller SAfiles inventory struct for this mop 
...
if exist(matfile,'file')
 fprintf('Loading SGfiles from %s\n',matfile);
 load([ mpath 'M'  num2str( m , '%5.5i' )  'SG.mat' ],'SGfiles');
 NumSGsurveys=size(SGfiles,2);
else
    fprintf('%s does not exist, making new SG struct array.\n',matfile);
    SGfiles=[];
    SG=[];
    NumSGsurveys=0;
end
...
idx=[]; 
% for s=NumSGsurveys:-1:1
%     % find same file name and creation date in Survey if it exists
%     idx=find(strcmp({Survey(ndx).File},SGfiles(s).File) &...
%         [Survey(ndx).FileDatenum] == SGfiles(s).FileDatenum);
%     if numel(idx) == 0 
%         break
%     end
% end
%% find surveys gridding since Mar 2023 in order to regrid with
%   standard 2d gridding code
if ~isempty(SGfiles)
idx=find([SGfiles.FileDatenum] > datenum(2023,3,1));
end

if numel(idx) > 0 
    
 fprintf('Changes found. Loading SG struct array from %s\n',matfile);
 load([ mpath 'M'  num2str( m , '%5.5i' )  'SG.mat' ],'SG');

 % remove entries after Mar 1 23
  bdx=find([SG.FileDatenum] > datenum(2023,3,1));
  fprintf('Removing %i SG entries after 1 Mar 23\n',numel(bdx));
  SG(bdx)=[];


 NumSGsurveys=size(SG,2); 
...
%   if ~isempty(ndx)
%        % load  Mop SG struct array if it exists
%        matfile=[mpath 'M' num2str(MopNumber,'%5.5i') 'SG.mat'];
%        if exist(matfile,'file')
%          load(matfile,'SG');
%        else
%            fprintf('Mop %i \n',MopID)
%            fprintf('Making New SG struct Array from scratch. \n',MopID)
%            SG=[];
%        end
       
   %ndx=find(cellfun(@(v)any(v(:) == MopNumber),{Survey.NearestMops}));
   % find any survey entries in the existing SG array that are no longer
   %  part of the gridable survey list
   if ~isempty(SG)
   ngone=find(~ismember({SG.File},{Survey(ndx).File})); 
   ...
   %ndx=find(cellfun(@(v)any(v(:) > MopNumber-3 & v(:) < MopNumber+3),{Survey.NearestMops}));
 if ~isempty(SG)
   nnew=find(~ismember({Survey(ndx).File},{SG.File}));
 else
   nnew=1:numel(ndx); % no existing SG array, grid all the surveys
 end
   
  if ~isempty(nnew)
      fprintf('Mop %i \n',MopID)
      fprintf('%i new files exist for this Mop.\n',numel(nnew));
