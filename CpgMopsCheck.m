...
% load the existing survey master list in the form of
%   a struct array called Survey, and keep in a struct
%   array called OldSurvey
%   
...
% idx=find([OldSurvey.Datenum] > datenum(2022,7,5));
% OldSurvey(idx)=[];
...
for nn=1:size(DataSets,2) % number of file data sets
...
if numel(idx2) > 0
    for n=idx2
        ...
        idx3=find([OldSurvey(idx1).Datenum] == Survey(n).Datenum & [OldSurvey(idx1).Bytes] == Survey(n).Bytes);
        if ~isempty(idx3)
           fprintf('\n%s\n survey date, source, and file size matches:\n',Survey(n).File)
           for m=idx3
               fprintf('%s\n',OldSurvey(idx1(m)).File)
           end
        end
    end
end
...
% and this is a valid survey check.
    if exist('/volumes/group','dir')  && exist('/volumes/drone','dir')
        
    fprintf('Saving Survey struct array to CheckSurvey.mat\n')
    fprintf('To update the CGP Mops with this info, run CpgMopsUpdate.m\n')
    save CheckSurvey.mat Survey
