...
% san dieguito + solana
MopStart=find(strcmp([Mop.Name],'D0620'));
%MopEnd=find(strcmp([Mop.Name],'D0646'));
MopEnd=find(strcmp([Mop.Name],'D0678'));
...
%figure('position',[346   118   585   635]);


PrevMon=0;
nn=1;
ns=0;
for surv=sdx(end-4:end)
...
Anom=Zmon-Zmean;
Anom(Anom <= 0)=NaN; % track only positive anomaly evolution

if nn == 1
    AnomPrev=Anom;
    nn = nn+1;
else
    AnomChange=Anom-AnomPrev;
    AnomPrev=Anom;

    %AnomChange(AnomChange < 0)=0;
mdx=find(~isnan(AnomChange(:)));
...
ax0=axes;
[ScatterPlot,ColorBarPlot]=ColorScatterPolarmap(lon,lat,AnomChange(mdx));
delete(ColorBarPlot)
hold on
...
t1=text(-117.277,32.9997,datestr(CS(idx).Datenum),...
    'fontsize',35,'fontweight','bold','color','k','backgroundcolor',[.8 .8 .8]);
tl=text(-117.2831+0.001,mean([32.9968   33.0022]),{'Time of Year','  Positive','  Elevation','  Anomaly','CHANGE (m)'},'color','w','fontsize',16,'horizontalalign','left');
...
set(gca,'xlim',[ -117.2831 -117.2728]);
set(gca,'ylim',[32.9968   33.0022]);
...
set(cb,'position',[0.07  0.0747    0.015    0.8505])
...
if nn == 2
    gif('TabletopZoomAnomalyChangeLast4.gif','overwrite',true,'DelayTime',.5)%,'resolution',72,'nodither')
    nn=nn+1;
