...
MopStart=find(strcmp([Mop.Name],'D0910'));
...
%%
cdx=find(strcmp({CS.Source},'USACE'));
cdx=cdx(6:end);
%cdx=cdx(1:5);
SA=CS(cdx);
...
title({[datestr(SA(1).Datenum) ' to ' datestr(CS(idx(1)).Datenum) ' |  SHOALS v Multibeam Change'],...
    ['Mean Change ' num2str(bias,'%5.3f') ' m.']},...
    'fontsize',16)
makepng(['OceansideMultibeam' datestr(CS(idx(1)).Datenum,'YYYYmmDD') 'vsShoals' datestr(SA(1).Datenum,'YYYYmmDD') 'Mops' num2str(MopStart) 'to' num2str(MopEnd) 'Change.png'])
...
title({[datestr(SA(1).Datenum) ' to ' datestr(CS(idx(1)).Datenum) ' | Shoals v Multibeam Change'],...
    ...
    'fontsize',16)
makepng(['OceansideMultibeam' datestr(CS(idx(1)).Datenum,'YYYYmmDD') 'vsShoals' datestr(SA(1).Datenum,'YYYYmmDD') 'DebiasedChange.png'])
...
ylabel([datestr(CS(cdx(end)).Datenum) ' SHOALS Depth (m, NAVD88)']);
title({'Oceanside Multibeam vs SHOALS :',['Comparison of N= ' num2str(N) ' overlapping 1m spatially-averaged depths']},'fontsize',16)
...
makepng(['OceansideMultibeam' datestr(CS(idx(1)).Datenum,'YYYYmmDD') ...
    'VsShoals' datestr(SA(1).Datenum,'YYYYmmDD') 'Scatterplot.png'])
