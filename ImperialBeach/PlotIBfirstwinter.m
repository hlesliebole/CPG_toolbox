...
if MopNumber == 51
    ...
    %title({'Imperial Beach 2012 Nourishment First Year Profile Evolution','(Center 1km Alongshore Average)'},'fontsize',22)
    title({'Imperial Beach 2012 Nourishment First Year Profile Evolution'},'fontsize',22)
...
if MopNumber == 655%658
    ...
    for nn=1:2:size(zt,1)
        nc=nc+1;
        if nn == 1
            %plot(0:149,zt(nn,:),':','color',col(nc,:),'linewidth',3,'DisplayName',['Fletcher Area ' char(string(Zdatetime(nn)))]);
            hold on;
        else
          %plt(nc)=plot(0:149,zt(nn,:),'.-','color',col(nc,:),'linewidth',3,'DisplayName',string(Zdatetime(nn)));
          hold on;  
...
makepng('IBnourishmentFirstYearProfile.png')
