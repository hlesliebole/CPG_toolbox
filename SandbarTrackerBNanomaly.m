...
GZm=Zmean( Zmean > zmin & Zmean < zmax ); % fixed section of global mean profile
Xm=SM(1).X1D( Zmean > zmin & Zmean < zmax ); % fixed section xshore distances
...
for njumbo=1:numel(ijumbo)
    ...
    Z=Z(ismember(X,Xm)); % fixed profile range
    Zanom=Z-GZm; % elevation anomaly
    
    
    m=Zanom;
    idx=find(~isnan(m) & m > 0 & GZm < 0);
    if ~isempty(idx);Xc=sum(m(idx).*Xm(idx))/sum(m(idx));else;Xc=NaN;end
    [~,ixmin]=min(abs(Xm-Xc));
    Gcsa=GZm(ixmin);
    Xcsa=Xc;
    ...
    zscaled = 1+nc*(Zanom-bmin)/brange;
    zscaled(zscaled < 1)=1;zscaled(zscaled > nc)=nc;
    idx=find(~isnan(zscaled)); % non NaN points 
    scp=scatter(Xm(idx), y(idx), 15, cm(ceil(zscaled(idx)),:), 'filled');
    if min(Z) < -5
    p1=plot(Xcsa,y(1),'m.','markersize',15);%plot(Gcn,y(1),'mo');
    p2=plot(Xcp,y(1),'ko','markersize',5);
    end
...
set(gca,'xlim',[min(Xm) max(Xm)]);
x0=max(Xm(GZm > 0));hold on;
%x0=0;
...
text(100,datenum(2022,10,1),'SUBAERIAL','fontsize',12,'fontweight','bold')
text(350,datenum(2022,10,1),'SUBAQUEOUS','fontsize',12,'fontweight','bold')

pos=get(gca,'position');
set(gca,'position',[pos(1) pos(2)+.06 pos(3) pos(4)-.06])
pos=get(gca,'position');
ax2=axes('position',[pos(1) pos(2)-.07 pos(3) 0]);
set(ax2,'xlim',[min(Xm) max(Xm)],'xdir','reverse','fontsize',12);
