...
mm=mm+1;
axes('position',[.05+(mm-1)*.12 .1 .11 .6])
% set a mop number
MopNumber=634+nn;
...
idx=find([SA.Datenum] > datenum(2024,4,15)); % plot all surveys
...
set(gca,'ylim',[-1 5],'xlim',[0 100],'fontsize',14);
xl=get(gca,'xlim');
...
if mm == 7
 ...
 set(gca,'position',[0.7700    0.1000    0.11    0.6000]);
end

box on;grid on;
title(['Mop ' num2str(MopNumber) ' Profiles']);
if mm == 4;xlabel('Xshore Distance (m)');end
if mm == 1;ylabel('Elev (m, NAVD88)');end
end
