...
title('Solana Beach Nourishment : 1 May vs 16 Apr 2024','fontsize',22)
xl=get(gca,'xlim');
hold on;
for n=3:31
...
load SolanaShoreboxMap.mat
idx=find(Z(:) > -12.5 & ~isnan(Z0(:)) & ~isnan(Z2(:)));
dZ=Z*NaN;
dZ(idx)=Z(idx)-Z0(idx);
dZ2=Z*NaN;
dZ2(idx)=Z2(idx)-Z0(idx);
...
% 
% sdate=datenum(2024,3,25);
% 
%     Z2=Z*nan;
% ndx=find((strcmp({CS.Source},'Trk') | strcmp({CS.Source},'AtvMR')) & [CS.Datenum] == sdate);
% for idx=ndx 
%     zdx=[];
...
% %kdx=find(Xsb > 500);Xsb=Xsb(kdx);Ysb=Ysb(kdx);zdx=zdx(kdx); % clip off san dieguito inlet area
% gdx=sub2ind(size(X),round(Ysb)-ygimg(1)+1,round(Xsb)-xgimg(1)+1);% 1d grid indices with data 
%     Z2(gdx)=CS(idx).Z(zdx); % overlay on grid
%     end
% end
hold on;
imagesc(xgimg,ygimg,dZ,'AlphaData',~isnan(dZ));hold on;
...
p2=plot(xgimg,sum(dZ2,'omitnan'),'-','color',[0.8500 0.3250 0.0980],'linewidth',2);hold on;
p1=plot(xgimg,sum(dZ,'omitnan'),'b-','linewidth',2);hold on;
plot(xgimg,0*sum(dZ,'omitnan'),'k:','linewidth',1);
set(gca,'fontsize',16);grid on;set(gca,'ylim',[-30 100],'ytick',-20:20:100,'xlim',[xgimg(1) xgimg(end)])
xlabel('Alongshore Distance (m)','fontsize',18);
ylabel({'Vol Change','(m^{3}/m-shore)'})
str=num2str(round(sum(sum(dZ,'omitnan'))),'%i');
str = regexprep(str,'(?<!\.\d*)\d{1,3}(?=(\d{3})+\>)','$&,');

str2=num2str(round(sum(sum(dZ(:,700:end),'omitnan'))),'%i');
...
legend([p2 p1],'16 Apr to 26 Apr','16 Apr to 01 May','location','northeast')
...
b1=plot(uz,dztot/1000,'b-','linewidth',2);hold on;
...
% array of total change in z that fall into each unique current z combination
dztot = accumarray(zidz(:), dz.');
b2=plot(uz,dztot/1000,'-','color',[0.8500 0.3250 0.0980],'linewidth',2);

ytickformat('%g K')
set(gca,'xlim',[-12 5],'ylim',[-.75 .5])
...
set(gca,'ytick',-0.75:.25:.5)
text(-11,-.25,{'Volume Change vs. 16 Apr Elevations','       Dog Beach Excluded '},...
    'backgroundcolor','w','fontsize',22)
hold on;
plot([-.83 -.83],[-.75 .5],'k--','linewidth',2)
plot([.79 .79],[-.75 .5],'k--','linewidth',2)
text([-.88],[.5],'MLLW','fontsize',14,'HorizontalAlignment','right')
text([.86],[.5],'MHHW','fontsize',14,'HorizontalAlignment','left')

set(gcf,'color','w')
set(gcf,'inverthardcopy','off')
set(gcf,'PaperPositionMode','auto');
legend([b2 b1],'16 Apr to 26 Apr','16 Apr to 01 May','location','south')
print(gcf,'-dpng','-r300','-loose','SolanaNourishment01MayVs16Apr24.png')
%%
...
% 
% makepng('SolanaPostnourishmentElevationChange.png')
