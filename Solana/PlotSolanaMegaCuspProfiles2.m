...
load SolanaShoreboxMap.mat
idx=find(Z0(:) > -12.5 & Z0(:) < 6 & ~isnan(Z0(:)) & ~isnan(Z1(:)) & ~isnan(Z2(:)) & ~isnan(Z3(:)) & ~isnan(Z4(:)) & ~isnan(Z5(:)) & ~isnan(Z6(:)) & ~isnan(Z7(:)));
...
dZL=Z0*NaN;dZL(idx)=Z7(idx)-Z6(idx)-Z6dz+Z7dz;

%hold on;
% imagesc(xgimg,ygimg,dZL,'AlphaData',~isnan(dZ7));hold on;
...
%     'fontweight','bold','backgroundcolor',[.7 .7 .7]);
% title('Solana Beach Nourishment Pad Evolution :  11 Jun (ATV Miniranger) vs 07 Jun 2024','fontsize',20)
% 
% for n=[11 17 21 29]
% plot([MopSB.BackLat(n) MopSB.OffLat(n)],[MopSB.BackLon(n) MopSB.OffLon(n)],'m-','linewidth',2)
% plot(MopSB.OffLon(n),MopSB.OffLat(n),'y.','markersize',15)
...
imagesc(xgimg,ygimg,dZ7,'AlphaData',~isnan(dZ5));hold on;
set(gca,'clim',[-1 1]);colormap(flipud(polarmap));cb=colorbar;
cb.Label.String='Elevation Change (m)';
 cb.Ticks=[-1:.2:1];
set(gca,'ylim',[-200 850]);
...
for n=[11 14 17 19  21 25 28]
%for n=[11 14 17 18  20 25 28]
plot([MopSB.BackLon(n) MopSB.OffLon(n)],[MopSB.BackLat(n) MopSB.OffLat(n)],'m-','linewidth',2)
text(MopSB.OffLat(n)+300,MopSB.OffLon(n),[ '   ' MopSB.Name{n}],'color','y','fontsize',16,'fontweight','bold')
text(MopSB.BackLon(n),MopSB.BackLat(n)-5,[ '  ' MopSB.Name{n}],'color','k','fontsize',16,'fontweight','bold','rotation',-90,'backgroundcolor',[.7 .7 .7])
end

mm=0;
for nn=[11 14 17 19  21 25 28]
%for nn=[8 14 17 19  21 25 27]
mm=mm+1;
axes('position',[.05 .055+(mm-1)*.095 .8 .09])
% set a mop number
MopNumber=634+nn;
...
idx=find([SA.Datenum] > datenum(2023,12,1)); % plot all surveys
...
for n=idx
 m=m+1;
 pl(m)=plot(X1D,Z1D(n,:),'-','color',col(m,:),'linewidth',1.5,'DisplayName',...
        datestr(SA(n).Datenum,'mm/dd/yy'));hold on;
end
set(gca,'ylim',[-1 5],'xlim',[0 100],'fontsize',14,'color',[.8 .8 .8]);
xl=get(gca,'xlim');
plot(xl,xl*0+1.344,'k--');
if mm == 4;text(xl(2)-5,1.55,'MHW','fontsize',12);end
set(gca,'xdir','reverse');
if mm == 4
 lg=legend(pl,'location','eastoutside','numcolumns',1);
 title(lg,'Survey Dates')
 lg.Position=[ 0.8678    0.0473    0.0663    0.6897];
 set(gca,'position',[0.0500    0.3400    0.8000    0.0900]);
end

box on;grid on;
text(98,3,['Mop ' num2str(MopNumber)],'fontsize',16);
if mm == 1;xlabel('Xshore Distance (m)');else set(gca,'xticklabels',[]);end
if mm == 4;ylabel('Elev (m, NAVD88)');end

set(gca,'dataaspectratio',[1 1 1])
end

makepng('SolanaCuspProfiles.png')
% %clabel(C,h)
...
% lg=legend([b1 b2 b3 b4 b5 b6 b7],'location','eastoutside','fontsize',14,'fontname','Monospaced');
% title(lg,'Date [z offset(cm)*]         South         Pad Reach^{**}          North ')
% print(gcf,'-dpng','-r300','-loose','SolanaNourishment11JunVs16Apr24.png')
