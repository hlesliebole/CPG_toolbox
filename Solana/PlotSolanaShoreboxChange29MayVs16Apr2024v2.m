...
load SolanaShoreboxMap.mat
idx=find(Z0(:) > -12.5 & Z0(:) < 6 & ~isnan(Z0(:)) & ~isnan(Z1(:)) & ~isnan(Z2(:)) & ~isnan(Z3(:))  & ~isnan(Z4(:))   & ~isnan(Z5(:)));
...
dZL=Z0*NaN;dZL(idx)=Z5(idx)-Z4(idx)-Z4dz+Z5dz;
...
text(1750,-10,{'Pad Reach'},'horizontalalign','center','fontsize',20,'color','y',...
    'fontweight','bold','backgroundcolor',[.7 .7 .7]);
title('Solana Beach Nourishment Pad Evolution :  29 May (ATV Miniranger) vs 24 May 2024','fontsize',20)
...
imagesc(xgimg,ygimg,dZ5,'AlphaData',~isnan(dZ5));hold on;
set(gca,'clim',[-1 1]);colormap(flipud(polarmap));cb=colorbar;
cb.Label.String='Elevation Change (m)';
 cb.Ticks=[-1:.2:1];
set(gca,'ylim',[-200 850]);
...
text(1750,-10,{'Pad Reach'},'horizontalalign','center','fontsize',20,'color','y',...
    'fontweight','bold','backgroundcolor',[.7 .7 .7]);
title('Solana Beach Nourishment Pad Evolution :  29 May (ATV Miniranger) vs 16 Apr 2024','fontsize',20)
...
text(-11.5,-.25,{'Pad Reach Volume Change','   vs. 16 Apr Elevations'},...
    'backgroundcolor','w','fontsize',18)
hold on;
text(16.2,.52,'Reach Volume Change (m^{3})','fontsize',22)
text(6.,-.48,'( )=Change from Previous Survey ;** Pad Terrace Excluded from Vol Change Estimate','fontsize',18)
%text(6.,-.55,'** Pad Terrace Excluded from Vol Change Estimate','fontsize',18)
text(6.,-.6,'* Z Datum Offset Based on Mean Pad Terrace Elevations','fontsize',18)
...
lg=legend([b1 b2 b3 b4 b5],'location','eastoutside','fontsize',14,'fontname','Monospaced');
title(lg,'Date [z offset(cm)*]         South         Pad Reach^{**}          North ')
print(gcf,'-dpng','-r300','-loose','SolanaNourishment29MayVs16Apr24.png')
