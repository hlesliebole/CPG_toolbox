...
title('Solana Beach Nourishment : 26 Apr vs 16 Apr 2024','fontsize',22)
xl=get(gca,'xlim');
hold on;
for n=3:31
...
plot(xgimg,sum(dZ,'omitnan'),'k-','linewidth',2);hold on;
plot(xgimg,0*sum(dZ,'omitnan'),'k:','linewidth',1);
set(gca,'fontsize',16);grid on;set(gca,'ylim',[-20 100],'ytick',-20:20:100,'xlim',[xgimg(1) xgimg(end)])
xlabel('Alongshore Distance (m)','fontsize',18);
ylabel({'Vol Change','(m^{3}/m-shore)'})
str=num2str(round(sum(sum(dZ,'omitnan'))),'%i');
str = regexprep(str,'(?<!\.\d*)\d{1,3}(?=(\d{3})+\>)','$&,');

text(500,60,['Total Change ' str ' m^3'],...
...
set(gca,'ytick',0:.5:2)
text(-11,1.5,{'Volume Change vs. Initial Elevations'},...
    'backgroundcolor','w','fontsize',22)
hold on;
plot([-.83 -.83],[0 2],'k--','linewidth',2)
plot([.79 .79],[0 2],'k--','linewidth',2)
text([-.88],[1.5],'MLLW','fontsize',14,'HorizontalAlignment','right')
text([.86],[1.5],'MHHW','fontsize',14,'HorizontalAlignment','left')

set(gcf,'color','w')
set(gcf,'inverthardcopy','off')
set(gcf,'PaperPositionMode','auto');
print(gcf,'-dpng','-r300','-loose','SolanaNourishment26AprVs16Apr24.png')
%%
