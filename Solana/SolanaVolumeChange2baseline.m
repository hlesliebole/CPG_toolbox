...
idx=find(~isnan(Z(:)));
    Z=griddata(double(X(idx)),double(Y(idx)),double(Z(idx)),...
...
% sdate(1)=datenum(2023,12,8);
% sdate(2)=datenum(2023,12,13);
% sdate(3)=datenum(2023,12,28);
sdate(1)=datenum(2024,1,9);
% %sdate(5)=datenum(2024,1,19);
% sdate(5)=datenum(2024,2,8);
% sdate(6)=datenum(2024,2,12);
% sdate(7)=datenum(2024,2,21);
% sdate(8)=datenum(2024,2,26);
% sdate(9)=datenum(2024,3,14);
% sdate(10)=datenum(2024,3,21);
% sdate(11)=datenum(2024,3,25);

% sdate(1)=datenum(2024,4,10);
% sdate(2)=datenum(2024,4,18);
% sdate(3)=datenum(2024,3,25);

Z2=Z*nan;
for ns=1:1   
%ndx=find((strcmp({CS.Source},'Trk') | strcmp({CS.Source},'AtvMR') | strcmp({CS.Source},'Gps')) & [CS.Datenum] == sdate(ns));
ndx=find((strcmp({CS.Source},'Trk') | strcmp({CS.Source},'AtvMR')) & [CS.Datenum] == sdate(ns));
for idx=ndx 
    zdx=[];
    if strcmp({CS(idx).Source},'Trk')
        zdx=find(CS(idx).Z > -1);
    elseif strcmp({CS(idx).Source},'Gps')  
        zdx=find(CS(idx).Z > -12);% & CS(idx).Z < 0.7);
    else
...
% idx=find(~isnan(Z2(:)));
%     Z2=griddata(double(X(idx)),double(Y(idx)),double(Z2(idx)),...
%         double(xgimg),double(ygimg));
% 
% nsurv=numel(find(~isnan(Z2(:))));
% nvalid=numel(find(~isnan(Z2(:)) & ~isnan(Z(:)) ));
% dV=sum(Z2(:)-Z(:),'omitnan');
% Zpad=Z(:,1050:3000);
% Z2pad=Z2(:,1050:3000);
% dVpad=sum(Z2pad(:)-Zpad(:),'omitnan');
% 
% 
% %fprintf('%s %8i  %8i %4.2f %8i\n',datestr(sdate(ns)),nsurv,nvalid,nsurv/nvalid,round(dV))
% fprintf('%s %8i %8i %8i\n',datestr(sdate(ns)),nsurv,round(dV),round(dVpad))
%%
