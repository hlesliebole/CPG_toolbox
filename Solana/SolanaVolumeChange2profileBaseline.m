%  in this version of building grids from combined lidar and gps data
%  
...
for m=str2num(MopSB.Name{4}(2:5)):2:str2num(MopSB.Name{end-1}(2:5))
    matfile=['M00' num2str(m,'%3.3i') 'SA.mat'];
    load(matfile,'SA')
    % identify Gps (dolly-jetski) and Atv miniranger surveys
    ndx=find(strcmp({SA.Source},'Gps') & [SA.Datenum] == sdate(1));
    adx=find(strcmp({SA.Source},'AtvMR') & [SA.Datenum] == sdate());
    ...
    [Xsb,Ysb]=utm2ShoreBox(Xutm,Yutm,str2num(MopSB.Name{1}(2:5)),str2num(MopSB.Name{end}(2:5)));
    % add points to shorebox grid indices
    gdx=sub2ind(size(X),round(Ysb)-ygimg(1)+1,round(Xsb)-xgimg(1)+1);% 1d grid indices with data 
    Z(gdx)=Z1D; % overlay on grid
end

%%

% Grid the combined elevation surveys using Delaunay tesselation 
maxdist=250; %set distance threshold of Nan boundary points for 200m spaced profiles 
idx=find(~isnan(Z(:)));
...
%%  3.  ATVmr in xg,yg space is gridded separately with a 5m NaN boundary tolerance
%     and overlaid on top of the gridded profiles
...
Z2=Z*nan;
for ns=1:1   
ndx=find((strcmp({CS.Source},'Trk') | strcmp({CS.Source},'AtvMR')) & ...
             [CS.Datenum] == sdate(ns));
for idx=ndx   
 zdx=find(CS(idx).Z > 0.72);
 if numel(zdx) > 0
...
colormap(jet);
%%
save SolanaPrenourishmentGrid.mat X Y Z0
