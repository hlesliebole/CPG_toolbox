...
MopSB=Mop(MopStart:MopEnd,:);
for n=1:18
plot([MopSB.BackLon(n) MopSB.OffLon(n)],[MopSB.BackLat(n) MopSB.OffLat(n)],'m-','linewidth',2)
plot(MopSB.BackLon(n)+0.002,MopSB.BackLat(n),'k.','markersize',1)
text(MopSB.BackLon(n),MopSB.BackLat(n),MopSB.Name{n},'color','c','fontsize',15,'fontweight','bold')
end
