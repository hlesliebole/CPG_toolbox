...
title('Solana Beach Nourishment : 16 Apr 2024','fontsize',22)
xl=get(gca,'xlim');
hold on;
for n=3:31
...
set(gca,'clim',[-5 5]);colormap(flipud(polarmap));cb=colorbar;
cb.Label.String='Change from PreNourishment Baseline (m)';
 cb.Ticks=[-5:1:5];
set(gca,'ylim',[-200 850]);
set(gca,'position',[0.1259    0.3100    0.7503    0.8150]);
[C,h] =contour(xgimg,ygimg,Z0-0.775,[0 -4 -10],'m:','linewidth',2);
...
plot(xgimg,sum(dZ,'omitnan'),'k-','linewidth',2);
...
text(-50,250,{'Total Change',[str ' m^3']},...
...
bar(uz-0.774,dztot/1000); % plot relative to msl
ytickformat('%g K')
set(gca,'xlim',[-12 5],'ylim',[-5 40])
set(gca,'fontsize',16);grid on;%set(gca,'ytick',0:100:400,'xlim',[xgimg(1) xgimg(end)])
xlabel('PreNourishment Elevation (m, MSL)','fontsize',18);
ylabel({'Vol Change (m^{3})'})
set(gca,'ytick',0:10:40)
text(-11,30,{'Volume Change vs. Initial Elevations'},...
    'backgroundcolor','w','fontsize',22)
hold on;
plot([-.83 -.83],[0 40],'k--','linewidth',2)
plot([.79 .79],[0 40],'k--','linewidth',2)
text([-.88],[35],'MLLW','fontsize',14,'HorizontalAlignment','right')
text([.86],[35],'MHHW','fontsize',14,'HorizontalAlignment','left')
...
print(gcf,'-dpng','-r300','-loose','SolanaNourishment16Apr2024.png')
%%
