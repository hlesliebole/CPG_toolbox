...
load SolanaShoreboxMap.mat
idx=find(Z0(:) > -12.5 & Z0(:) < 6 & ~isnan(Z0(:)) & ~isnan(Z1(:)) & ~isnan(Z2(:)) & ~isnan(Z3(:)) & ~isnan(Z4(:)) & ~isnan(Z5(:)) & ~isnan(Z6(:)));
...
dZL=Z0*NaN;dZL(idx)=Z6(idx)-Z5(idx)-Z5dz+Z6dz;

hold on;
imagesc(xgimg,ygimg,dZL,'AlphaData',~isnan(dZ6));hold on;
...
text(1750,-10,{'Pad Reach'},'horizontalalign','center','fontsize',20,'color','y',...
    'fontweight','bold','backgroundcolor',[.7 .7 .7]);
title('Solana Beach Nourishment Pad Evolution :  07 Jun (ATV Miniranger) vs 29 May 2024','fontsize',20)
...
imagesc(xgimg,ygimg,dZ6,'AlphaData',~isnan(dZ5));hold on;
set(gca,'clim',[-1 1]);colormap(flipud(polarmap));cb=colorbar;
cb.Label.String='Elevation Change (m)';
 cb.Ticks=[-1:.2:1];
set(gca,'ylim',[-200 850]);
...
text(1750,-10,{'Pad Reach'},'horizontalalign','center','fontsize',20,'color','y',...
    'fontweight','bold','backgroundcolor',[.7 .7 .7]);
title('Solana Beach Nourishment Pad Evolution :  07 Jun (ATV Miniranger) vs 16 Apr 2024','fontsize',20)
...
lg=legend([b1 b2 b3 b4 b5 b6],'location','eastoutside','fontsize',14,'fontname','Monospaced');
title(lg,'Date [z offset(cm)*]         South         Pad Reach^{**}          North ')
print(gcf,'-dpng','-r300','-loose','SolanaNourishment07JunVs16Apr24.png')
