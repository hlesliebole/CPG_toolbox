...
title('Solana Beach Nourishment Pad Evolution :  24 May (ATV Miniranger) vs 16 Apr 2024','fontsize',22)
xl=get(gca,'xlim');
hold on;
for n=3:31
...
load SolanaShoreboxMap.mat
idx=find(Z0(:) > -12.5 & Z0(:) < 6 & ~isnan(Z0(:)) & ~isnan(Z1(:)) & ~isnan(Z2(:)) & ~isnan(Z3(:))  & ~isnan(Z4(:)));
...
dZL=Z0*NaN;dZL(idx)=Z4(idx)-Z3(idx)+Z3dz+Z4dz;

hold on;
imagesc(xgimg,ygimg,dZL,'AlphaData',~isnan(dZ4));hold on;
...
b1=plot(uz-0.774,dztot/1000,'-','linewidth',2,'displayname',...
    ['16 Apr to 26 Apr[' num2str(-Z1dz*100,'%3.1f') ']' SP1 '             ' P1 '               ' NP1]);hold on;
...
b2=plot(uz-0.774,dztot/1000,'-','linewidth',2,'displayname',...
    ['16 Apr to 01 May[' num2str(-Z2dz*100,'%3.1f') ']' SP2 dSP2 '  ' P2 dP2 '  ' NP2 dNP2]);hold on;
...
b3=plot(uz-0.774,dztot/1000,'-','linewidth',2,'displayname',...
    ['16 Apr to 10 May[' num2str(-Z3dz*100,'%3.1f') ']' SP3 dSP3 '  ' P3 dP3 '  ' NP3 dNP3]);hold on;
...
b4=plot(uz-0.774,dztot/1000,'-','linewidth',2,'displayname',...
    ['16 Apr to 24 May[' num2str(-Z4dz*100,'%3.1f') ']' SP4 dSP4 ' ' P4 dP4 ' ' NP4 dNP4]);hold on;
...
hold on;
text(9.2,.55,'Reach Volume Change (m^{3})','fontsize',22)
text(6.,-.55,'( )=Change from Previous Survey ; ** Z Datum Error Based on Pad Terrace Elevations','fontsize',18)
...
lg=legend([b1 b2 b3 b4],'location','eastoutside','fontsize',16);
title(lg,'Date [z offset(cm)**]            South         Pad Reach^{**}          North ')
print(gcf,'-dpng','-r300','-loose','SolanaNourishment24MayVs16Apr24.png')
