...
% idx=find(contains({SM.File},'umbo'));
% SM=SM(idx);

% survey month
smonth=month(datetime([SA.Datenum],'convertfrom','datenum'));
%idx=find( (smonth == 4 | smonth == 5)  & contains({SA.File},'umbo'));
...
nn=0;
for n=1:nr 
    m=idx(numel(idx)-nr+n);
    %m=idx(m);
    [x1d,z1di]=GetNonGriddedProfile(MopNumber,m);
...
%     if n==3;SM(m).Datenum=datenum(2021,10,13);end
    ...
    nn=nn+1;
    z=z1di;%z(z <-1.0)=NaN;
    xMSL=intersections([x1d(1) z1di(end)],[0.774 0.774],x1d,z1di);
    if isempty(xMSL);xMSL=999.9;end
    if n == nr
        p(nn)=plot(x1d,z,'k-','linewidth',3,'DisplayName',...
        [datestr(SA(m).Datenum,'mm/dd/yy')]);hold on;
    else
       p(nn)=plot(x1d,z,'-','color',col(n,:),'linewidth',2,'DisplayName',...
        [datestr(SA(m).Datenum,'mm/dd/yy')]);hold on;
    end
    end
end
