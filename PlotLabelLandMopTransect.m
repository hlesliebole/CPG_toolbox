function PlotLabelLandMopTransect(MopID,PlotMethod,LineColor,ShadeOnOff)
...
% plot transect
 ...
 if contains(PlotMethod,'3d','IgnoreCase',true) % 3D plot
  ...
  if strcmpi(ShadeOnOff,'ShadeOn') && MopNumber > 2
    ...
    hold on;
    if Linecolor == 'y'
    text(Mop.BackLon(n),Mop.BackLat(n),zd,[num2str(MopNumber) '  '],...
        'horizontalalign','right','fontsize',14,'fontweight','bold',...
        'color',LineColor,'rotation',90)
    else
        text(Mop.BackLon(n),Mop.BackLat(n),zd,[num2str(MopNumber) '  '],...
        'horizontalalign','right','fontsize',14,'fontweight','bold',...
        'color',LineColor,'rotation',90)
...
% option to shade Mop area being viewed on 2D plot
  ...
  if strcmpi(ShadeOnOff,'ShadeOn') && MopNumber > 2
    ...
    if LineColor == 'y'
   text(Mop.BackLon(n),Mop.BackLat(n)+0.001,table2cell(Mop(n,1)),...
        'horizontalalign','left','fontsize',16,'fontweight','bold',...
        'color','k','rotation',90,'backgroundcolor','y')
    else
        text(Mop.BackLon(n),Mop.BackLat(n)+0.001,table2cell(Mop(n,1)),...
        'horizontalalign','left','fontsize',16,'fontweight','bold',...
        'color','k','rotation',90,'backgroundcolor','w')
    end
