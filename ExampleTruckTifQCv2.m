...
idx=find([TifFiles.MopStart] <= MopMax & [TifFiles.MopEnd] >= MopMin);
fprintf('%i tif files found for Mops %i\n',numel(idx),MopMin,MopMax)
...
title(['Mops ' num2str(MopMin) '-'  num2str(MopMax) ' Number of Values at each tif grid point']);
...
title(['Mops ' num2str(MopMin) '-'  num2str(MopMax) ' Minimum Tif Elevations']);
...
view(-20,20);xlabel('E UTM');ylabel('N UTM');zlabel('Elev (m, NAVD88)');
title(['Mops ' num2str(MopMin) '-'  num2str(MopMax) ' Maximum Tif Elevations']);

% use the matlab isoutlier function to flag suspect elevations at
% each tif grid point
TF=isoutlier(Zt);
TF=isoutlier(Zt,'mean','ThresholdFactor',3); % more the 3 sigma from mean
...
grid on;
view(0.01,0)
title(['Mops ' num2str(MopMin) '-'  num2str(MopMax) ' isoutliers'])

% list number of outliers by tif file name
TF=isoutlier(Zt,'mean','ThresholdFactor',4); % more the 4 sigma from mean
idx=find([TifFiles.MopStart] <= MopMax & [TifFiles.MopEnd] >= MopMin);
for n=1:numel(idx)
    fprintf('%i Bad Points: %s\n',numel(find(TF(n,:) == 1)),TifFiles(idx(n)).name)
end
