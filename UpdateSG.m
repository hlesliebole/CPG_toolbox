...
%for MopID=550:555% BN
for MopID=571:594% Ruby2d
%
    fprintf('----------------- \n')
    fprintf('Mop %i \n',MopID)
    fprintf('----------------- \n')
...
% first loop through SG file names and update any previously
...
%     
    ...
    % inner loop through nearby Mop surveys
      ...
      if ~isempty(N1)
       ...
       tidx=find(strcmpi({N1.SA.File}, SA(SurvNum).File)==1);
       if ~isempty(tidx)         
	    for ntidx=tidx'
              Xr=[Xr' N1.SA(ntidx).X']';
              Yr=[Yr' N1.SA(ntidx).Y']';
      ...
      if ~isempty(N2)
       ...
       tidx=find(strcmpi({N2.SA.File}, SA(SurvNum).File)==1);
       if ~isempty(tidx)         
	    for ntidx=tidx'
              Xr=[Xr' N2.SA(ntidx).X']';
              Yr=[Yr' N2.SA(ntidx).Y']';
      ...
      if ~isempty(N3)
       ...
       tidx=find(strcmpi({N3.SA.File}, SA(SurvNum).File)==1);
       if ~isempty(tidx)         
	    for ntidx=tidx'
              Xr=[Xr' N3.SA(ntidx).X']';
              Yr=[Yr' N3.SA(ntidx).Y']';
      ...
      if ~isempty(N4)
       ...
       tidx=find(strcmpi({N4.SA.File}, SA(SurvNum).File)==1);
       if ~isempty(tidx)         
	    for ntidx=tidx'
              Xr=[Xr' N4.SA(ntidx).X']';
              Yr=[Yr' N4.SA(ntidx).Y']';
...
for n=1:size(SG,2)
    if strcmp(SA(n).File,SG(n).File) == 0
        fprintf('Record %i Out of Sync:\nSA: %s\nSG: %s\n',...
            n,SA(n).File,SG(n).File)
        SGS=SG(n); % save out of sync survey record
        nsync=find(strcmp({SG.File},SA(n).File) == 1); % find synced record
        SG(n)=SG(nsync);SG(nsync)=SGS; % switch the two
        cflag = 1;
    end
end
