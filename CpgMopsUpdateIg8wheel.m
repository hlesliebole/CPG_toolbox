...
% % delete entries for old surveys after 7/5/2022
idx=find([Survey.Datenum] > datenum(2022,8,5));
Survey(idx)=[];

% Find iG8wheel filenames in the CheckSurvey struct array that
%  are not found in the Survey struct array 

%idx2=find(~ismember(lower({CheckSurvey.File}),lower({Survey.File})));
idx2=find(~ismember(lower({CheckSurvey.File}),lower({Survey.File})) & strcmp({CheckSurvey.Source},'iG8wheel'));
...
%      fprintf('        To: %s\n',CheckSurvey(n2).File)
%          
%      if(abs(Survey(n).Datenum-CheckSurvey(n2).Datenum) <= 1 ...
%              && strcmp(Survey(n).Source,CheckSurvey(n2).Source) == 1 ...
%              && Survey(n).Bytes == CheckSurvey(n2).Bytes)
%          fprintf(' Date/Source/Bytes Match, Updating File name and FileDatenum in Survey %s\n',Survey(n).File)
%             Survey(n).File=CheckSurvey(n2).File;
%             Survey(n).FileDatenum=CheckSurvey(n2).FileDatenum;
%      else
%          %fprintf('          No Match.\n')
%      end
%     end
...
% % Delete any Survey entries that are not in CheckSurvey
