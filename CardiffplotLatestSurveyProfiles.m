...
% 647 653 656 664 667
for MopNumber=654:654

load(['M00' num2str(MopNumber,'%3.3i') 'SA.mat'])
ndx=find([SA.Datenum] > datenum(2017,1,1) & ...
    [SA.Datenum] < datenum(2019,12,31) & ...
    contains({SA.File},'umbo'));
% fall surveys
...
%         else
       ...
       syr=year(datetime(SA(n).Datenum,'convertfrom','datenum'));
    if syr == 2017
        set(p(m),'color','g')
    elseif syr == 2018
        set(p(m),'color','b')
...
set(gca,'ylim',[-10 3.5],'xdir','reverse','fontsize',14);

title([{['Mop ' num2str(MopNumber)]},{['Surveys Since ' datestr(SA(ndx(1)).Datenum)]}],...
   'fontsize',16);
...
legend(p,'location','eastoutside','fontsize',9,'numcolumns',1);%ceil(size(SA,2)/60))
makepng(['CardiffMop' num2str(MopNumber) 'Profiles.png'])
