...
if exist(matfile,'file');load(matfile,'SG','SGfiles');end
...
nd=0;
% regrid GPS surveys
GpsSurv=find(strcmp({SG.Source},'Gps') | strcmp({SG.Source},'iG8wheel') );
%GpsSurv=find(strcmp({SG.Source},'Gps') & [SG.Datenum] > datenum(2019,1,1));

   for SurveyNum=GpsSurv%ndx(nadd)
       fprintf('Updating Grid for: %s\n',SG(SurveyNum).File)
...
%       
    % start aggregate x,y, z arrays 
         ...
         SurvNum=find(strcmpi({SA.File},SG(SurveyNum).File));
         
         if isempty(SurvNum)
             SurvNum=find(strcmpi({SA.Source},'Gps') &...
                 abs([SA.Datenum]-SG(SurveyNum).Datenum) < 7);
             if ~isempty(SurvNum);SurvNum=SurvNum(1);end
         end
...
%     if strcmpi(Survey(SurveyNum).Source,'Gps') == 1     
      if ~isempty(N1)
       ...
       tidx=find(strcmp({N1.SA.File},SG(SurveyNum).File) == 1);
       if isempty(tidx)
             tidx=find(strcmpi({N1.SA.Source},'Gps') &...
                 abs([N1.SA.Datenum]-SG(SurveyNum).Datenum) < 7);
             if ~isempty(tidx);tidx=tidx(1);end
       end
       if ~isempty(tidx) 
...
%     end
       ...
       if ~isempty(N2)
       % if it has a matching survey file, include its x,y,z points
       tidx=find(strcmp({N2.SA.File},SG(SurveyNum).File) == 1);
       if isempty(tidx)
             tidx=find(strcmpi({N2.SA.Source},'Gps') &...
                 abs([N2.SA.Datenum]-SG(SurveyNum).Datenum) < 7);
             if ~isempty(tidx);tidx=tidx(1);end
       end
       if ~isempty(tidx) 
       ...
       if ~isempty(N3)
       % if it has a matching survey file, include its x,y,z points
       tidx=find(strcmp({N3.SA.File},SG(SurveyNum).File) == 1);
       if isempty(tidx)
             tidx=find(strcmpi({N3.SA.Source},'Gps') &...
                 abs([N3.SA.Datenum]-SG(SurveyNum).Datenum) < 7);
             if ~isempty(tidx);tidx=tidx(1);end
       end
       if ~isempty(tidx) 
...
%     if strcmpi(Survey(SurveyNum).Source,'Gps') == 1    
       if ~isempty(N4)
       % if it has a matching survey file, include its x,y,z points
       tidx=find(strcmp({N4.SA.File},SG(SurveyNum).File) == 1);
       if isempty(tidx)
             tidx=find(strcmpi({N4.SA.Source},'Gps') &...
                 abs([N4.SA.Datenum]-SG(SurveyNum).Datenum) < 7);
             if ~isempty(tidx);tidx=tidx(1);end
       end
       if ~isempty(tidx) 
...
%      SG(nd).Datenum=Survey(SurveyNum).Datenum; 
     SG(SurveyNum).X=XGutm;
     SG(SurveyNum).Y=YGutm;
     SG(SurveyNum).Z=Vzg;
     
     % intialize grid point classification as unknown
     SG(SurveyNum).Class=zeros(size(XGutm));
     ...
     SG(SurveyNum).Class(li(l > 0))=Cr(l(l > 0));
