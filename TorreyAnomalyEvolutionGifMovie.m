...
MopStart=find(strcmp([Mop.Name],'D0576'));
...
DateEnd=datenum(2025,9,1);

CS=SGcombineMops(MopStart,MopEnd);
[CSjumbo, merges] = mergeSurveysJumbo(CS);
ndx=find(contains({CSjumbo.File},'umbo')&(strcmp({CSjumbo.Source},'Gps') | strcmp({CSjumbo.Source},'Jumbo'))...
    & [CSjumbo.Datenum] >= DateStart & [CSjumbo.Datenum] <= DateEnd);
%CSjumbo=CSjumbo(jdx);
...
figure('position',[ 292    57   549   740])
...
for surv=ndx
    ...
    Mon=month(datetime(CSjumbo(surv).Datenum,'convertfrom','datenum'));
    if Mon ~= PrevMon
    ...
    ndx=find((strcmp({CSjumbo.Source},'Gps')) &...
        [CSjumbo.Datenum] >= CSjumbo(surv).Datenum-2 & [CSjumbo.Datenum] <= CSjumbo(surv).Datenum+2);
    ...
    title([datestr(CSjumbo(idx).Datenum) ' Elevation Anomaly at Torrey Pines'],'fontsize',16);
    sdatenum=CSjumbo(idx).Datenum;
    %t1=text(-117.272,mean([32.9684   33.0028]),datestr(CS(idx).Datenum),'fontsize',35,'fontweight','bold','color','k','backgroundcolor',[.8 .8 .8]);
    % text(-117.285,mean([32.9684   33.0028]),{'Time of Year','  Elevation','Anomaly (m)'},'color','w','fontsize',16,'horizontalalign','left');
    ...
    set(gca,'xlim',[-117.268 -117.258]);
    set(gca,'ylim',[32.92   32.935]);
    ...
    if nn == 1
        gif('TorreyAnomalyFast2.gif','overwrite',true,'DelayTime',.5)
        nn=nn+1;
    else
        gif('frame',gcf)
        nn=nn+1;
