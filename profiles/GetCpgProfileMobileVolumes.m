function [BeachVolMin,BeachVolMax]=GetCpgProfileMobileVolumes(Znavd88,XbackgapTol,X1D,Z1D)

zmin=min(Z1D); % lowest elevations profile
...
for n=1:nsurv
 ...
 % identify the size of the gap each xshore point is in (0 = not in a gap)
     ...
     BackGapSize=idxlast-idxback; % gap between back beach point and first data point 
 
     % set gap points equal to first valid data point value if gap <=
     % XbackgapTol
     %fprintf('%i back gap %i\n',n,BackGapSize)
     if BackGapSize > 0 & BackGapSize <= XbackgapTol
 ...
 % if no gaps, calculate a volume
     ...
     BeachVolMin(n)=sum(Z1D(n,idxback:idxBW)-zmin(idxback:idxBW)); 
    end
   end
 ...
 % if no gaps, calculate a volume
     ...
     BeachVolMax(n)=sum(Z1D(n,idxback:idxBW)-zmin(idxback:idxBW)); 
    end
   end
