% Example code to QC SA survey data by building out backward and
% forwards in time from the most spatial dense survey
%  
...
% find survey with the most points between -8 and +3m
nmax=0;
maxpts=0;
for n=1:size(SA,2)
    %if min(SA(n).Z) < -3 && max(SA(n).Z) > 1 % full profile option
     %pts=numel(find(SA(n).Z >= -8 & SA(n).Z <= 3));
     pts=numel(find(SA(n).Z >= 0 & SA(n).Z <= 3)); % beach only option
     if pts > maxpts
        maxpts=pts;
        nmax=n;
     end
...
figure;ScatterPlotBeachUTM(SA(nmax).X,SA(nmax).Y,SA(nmax).Z,'3d');
grid on;colorbar;title([SA(nmax).Source ' ' datestr(SA(nmax).Datenum)])
...
ColorBarPlot.Location='eastoutside';
xl=get(gca,'xlim');
