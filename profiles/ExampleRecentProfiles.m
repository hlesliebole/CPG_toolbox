...
Nstep=1;
col=jet(NR/Nstep);
figure('position',[76          43        1298         722]);
nc=0;
for n=N-NR+Nstep:Nstep:N
    nc=nc+1;
    if n==N-NR+Nstep || n==N
        plot(X1Dt,Zf(n,:),'-','color',col(nc,:),'linewidth',3,...
        'DisplayName',datestr(SA(n).Datenum));hold on;
    else
    plot(X1Dt,Zf(n,:),'-','color',col(nc,:),...
        'DisplayName',datestr(SA(n).Datenum));hold on;
    end
end
set(gca,'xdir','reverse','xlim',[-20 120],'ylim',[-1 7]);grid on;
