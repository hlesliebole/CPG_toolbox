...
np=0;
 ...
 for MopNum=[536 582]%MopRng(sb,1):MopRng(sb,2)
  ...
  nq=0; % quarter and valid profile counters  
  for q=[4 1 2 3]
      nq=nq+1;
      ...
      % plot if it exists  
        if exist(pfile,'file')
          ...
          % plot the profile
          if MopNum == 582
              p(np)= plot(d(:,1)+30,d(:,6),':','color',col(nq,:),'linewidth',3,...
              'DisplayName',['MSL-matched Mop 582 ' DispName(nq,:)]);hold on;
          else
            p(np)=plot(d(:,1),d(:,6),'-','color',col(nq,:),'linewidth',3,...
              'DisplayName',['Mop 536 ' DispName(nq,:)]);hold on;
          end
  ...
  legend([p1 p2],'location','northwest','fontsize',16)
   ...
   print(gcf,'-djpeg','Mop536vs582profiles.jpg','-r72');
