...
%     end
% end
mm=find(strcmp({SM.Source},'Trk') | (strcmp({SM.Source},'AtvMR') & [SM.Datenum] == datenum(2024,5,24)));
col=jet(nr);
for n=1:nr    
    m=mm(numel(mm)-nr+n);
%     if n==1;SM(m).Datenum=datenum(2021,10,11);end
...
legend([p],'location','eastoutside','numcolumns',2);
...
makepng(['MOP' num2str(MopNumber) 'profiles.png'])
  ...
  makepng(['MOP' num2str(MopNumber) 'Last5profiles.png'])  
end
end  