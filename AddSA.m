%function [SA]=AddSA(MopNum,SurveyNum,Survey)
clear all
...
%for MopNum=1:11594%550:555%580:590%
for MopNum=550:555
%for MopNum=663:684
 %fprintf('Mop: %i\n',MopNum)  
 matfile=[mpath 'M' num2str(MopNum,'%5.5i') 'SA.mat'];
 if exist(matfile,'file')
      load(matfile,'SA');
      %fprintf('SA array for this Mop contains %i files.\n',size(SA,2));
 if(size(SA,2) > 0)
 % find Survey files that should have data for this Mop based on
 %  on the survey master list 
    ndx=find(cellfun(@(v)any(v(:) == MopNum),{Survey.NearestMops}));
    %fprintf('Survey list contains %i files for this Mop.\n',numel(ndx));
    if ~isempty(ndx) 
    ngone=find(~ismember({SA.File},{Survey(ndx).File}));    
    if ~isempty(ngone)
        fprintf('Mop: %i\n',MopNum);
        fprintf('%i files no longer exist for this Mop.\n',numel(ngone));
      for n=fliplr(ngone)
        fprintf('Removing entry for: %s\n',SA(n).File)
        SA(n)=[];
      end
      save(matfile,'SA');
    end

% see if any of these Surveys are not currently in the SA struct array
    nnew=find(~ismember({Survey(ndx).File},{SA.File}));
    
  if ~isempty(nnew)
      
      fprintf('Mop: %i\n',MopNum);
      fprintf('%i new files for this Mop.\n',numel(nnew));
      
   nd=size(SA,2);
   for SurveyNum=ndx(nnew)
     
     fprintf('Adding: %s\n',Survey(SurveyNum).File)
     [nsa]=UpdateSAsurvey(mpath,Mop,Survey,SurveyNum);
     % see if there are any mop numbers that don't match the saved list
     diffmops=setxor(nsa,[Survey(SurveyNum).NearestMops]);
     if ~isempty(diffmops)
       Survey(SurveyNum).NearestMops=nsa; % if so redefine list and save
       save SurveyMasterListWithMops.mat Survey
     end
       
% % read in data
%     [x,y,z,c,utmzone]=readSurveyFileUTM2(Survey(SurveyNum).Source,Survey(SurveyNum).File);
%    if ~isempty(x)
%     ext=Survey(SurveyNum).File(end-2:end); % get filename extension
%     if(strcmpi(ext,'tif') == 1 || strcmpi(Survey(SurveyNum).Source,'UTAir') == 1)
%     else
...
%     if ~isempty(x(nmop == MopNum))
%         nd=nd+1;  
%             SA(nd).Mopnum=MopNum;
%             SA(nd).UTMzone=utmzone(1,:);
%             SA(nd).File=Survey(SurveyNum).File;
%             SA(nd).FileDatenum=Survey(SurveyNum).FileDatenum;
%             SA(nd).Bytes=Survey(SurveyNum).Bytes;
%             SA(nd).Source=Survey(SurveyNum).Source;
%             SA(nd).Datenum=Survey(SurveyNum).Datenum;
%             SA(nd).X=x(nmop == MopNum);
%             SA(nd).Y=y(nmop == MopNum);
%             SA(nd).Z=z(nmop == MopNum);
%             SA(nd).Class=c(nmop == MopNum);
%     else
%       fprintf('File has no data for Mop %i. Editing/reSaving Survey struct array.\n',MopNum)  
%       % correct Survey struct array and resave
%       Survey(SurveyNum).NearestMops=unique(sort(nmop));
%       save SurveyMasterListWithMops.mat Survey
%     end
%   end
end
...
%     save([mpath 'M' num2str(MopNum,'%5.5i') 'SA.mat'],'SA');
