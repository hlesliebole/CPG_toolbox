...
Jdates=[datenum(2015,10,26) datenum(2016,4,4) datenum(2016,9,27)];

% figure out how many surveys to process starting 6 Apr
MopNumber=582;
load(['M00' num2str(MopNumber,'%3.3i') 'SM.mat'])
...
figure('position',[182   306   860   485])
for n=1:nsurveys
sn=find([SM(jumbo).Datenum] == Jdates(n));
x1d=SM(jumbo(sn)).X1D;
z1=SM(jumbo(sn)).Z1Dmedian;
...
% plot([220 620],[-4 -4],'k:')
% plot([220 220],[-12 -4],'k:')
set(gca,'xdir','reverse','xlim',[0 620],'fontsize',18);grid on;
legend(p,'location','north','fontsize',20)
title({'Torrey Pines 2015-16','Severe Winter Erosion and Partial Summer Recovery'},'fontsize',22);
xlabel('Cross-Shore Distance (m)');ylabel('Elevation (m, MSL)');
makepng('Torrey2016Profiles.png')

% fill([220 620 620 220 220],[-12 -12 -4 -4 -12],'y','facealpha',.2)
% legend(p,'location','north','fontsize',20)
% 
% text(570,-3.5,'Weak Recovery Below -4m MSL','fontsize',22)
% makepng('Torrey2016ProfilesHighlight.png')
