...
ibias=find(z1s-0.774 > -9.5 & z1s-0.774 < -9 & ~isnan(zmm));
bias=-mean(zmm(ibias));
zmm=zmm+bias;
...
hold on;
idx=find(z1s-0.774 > -9 & z1s-0.774 < -8);
v9(m)=mean(zmm(idx),'omitnan');
idx=find(z1s-0.774 > -8 & z1s-0.774 < -7);
v8(m)=mean(zmm(idx),'omitnan');
idx=find(z1s-0.774 > -7 & z1s-0.774 < -6);
v7(m)=mean(zmm(idx),'omitnan');
idx=find(z1s-0.774 > -6 & z1s-0.774 < -5);
v6(m)=mean(zmm(idx),'omitnan');
idx=find(z1s-0.774 > -5 & z1s-0.774 < -4);
v5(m)=mean(zmm(idx),'omitnan');
dv(m)=SA(n2).Datenum;
end
end
...
set(gca,'ylim',[-40 20],'ytick',-35:5:15,'xlim',[-11 -3]);
%set(gca,'ylim',[-40 40],'ytick',-35:5:35,'xlim',[-11 0]);
xl=get(gca,'xlim');plot(xl,[0 0],'k--','linewidth',2);
title([{'Mop 580-584 Average'},{['JetSki Survey Elev change since '...
...
makepng('TorreyRecoveryJetSkiChange.png')
