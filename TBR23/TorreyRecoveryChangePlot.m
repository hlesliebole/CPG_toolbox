...
n=size(SA,2);
     ...
     [x1d,z1di2]=GetNonGriddedProfile(MopNumber,n-1);
...
% m=m+1;
% hold on;p(m)=plot(SM(jumbo(end-2)).X1D,SM(jumbo(end-2)).Z1Dtransect',...
%     'k:','linewidth',3,...
%     'DisplayName',[num2str(587) ' ' datestr(SM(jumbo(end-2)).Datenum)]);
grid on;
box on;
set(gca,'linewidth',2);
set(gca,'fontsize',16);xlabel('Distance From Mop Back Beach Point (m)');
ylabel('Elevation Change (m, NAVD88)')
set(gca,'ylim',[-.5 .5],'xdir','reverse','fontsize',16);
set(gca,'ytick',-.5:.1:0.5,'xlim',[100 600]);
xl=get(gca,'xlim');plot(xl,[0 0],'k--','linewidth',2);
title([{'Mops 578-595'},{['JetSki Survey Elev change: '...
    datestr(SA(end-1).Datenum) ' to ' datestr(SA(end).Datenum) ]}],...
   'fontsize',18);
