...
mm=0;
col=jet(numel(507:512));
for MopNum=566%507:512
    mm=mm+1;
    Beta=[];
...
plot(x,BetaUpper,'ko','markersize',8,'markerfacecolor',col(mm,:),'linewidth',1,'DisplayName',['Mop ' num2str(MopNum)]);
...
% x=datetime([SA.Datenum],'convertfrom','datenum');
% plot(x,Beta,'b.','markersize',20,'linewidth',2,'DisplayName','Monthly {\beta}');
% plot(x,BetaLower,'.','color',[0 .8 0],'markersize',20,'linewidth',2,'DisplayName','Monthly {\beta_{Lower}}');
% plot(x,BetaUpper,'r.','markersize',20,'linewidth',2,'DisplayName','Monthly {\beta_{Upper}}');
set(gca,'fontsize',14,'xlim',[datetime(2022,10,15) datetime(2023,6,15)])
grid on;
legend('location','eastoutside')
makepng('SIOBeachFaceSlopes.png')
