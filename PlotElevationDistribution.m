...
figure('position',[ 108          16        1235         781]);
subplot(2,1,2)
...
av=sum(dztot(idx))/5;
dv=sum(dztot(uz > .7)/5);
figure;bar(uz,dztot);set(gca,'xlim')
...
plot([.774 .774],yl,'k--');text(.9,.95*yl(2),' MSL','fontsize',14,'horizontalalign','right');
set(gca,'fontsize',16)
title({[datestr(SA(end-1).Datenum) ' to ' datestr(SA(end).Datenum) ' Vol Change Above MSL'],[' ( erosion | accretion | net ): ' ....
    num2str(ev,'%4.1f') ' | +' num2str(av,'%4.1f') ' | ' ...
    num2str(dv,'%4.1f') 'm^{3}/m']},'fontsize',22);  
...
plot([.774 .774],yl,'k--');text(.9,.95*yl(2),' MSL','fontsize',14,'horizontalalign','right');
set(gca,'fontsize',16)
title([datestr(SA(end).Datenum) ' : SIO Subaerial Beach Elevation Spectrum (Mops 507-511)'],'fontsize',22)

makepng('SIOelevSpectrum.png')