...
fig=figure('Name','CPGMOP Profile Tool','NumberTitle','off',...
...
MopCellStr=strrep(strrep({SMf.name}','SM.mat',''),'M','');
...
Sdesc=[{'Date                            '},...
       {'MSL (.774m navd88) location     '},...
       {'Upper Shoreface (1.06m) location'},...
       {'MHW (1.34m) location            '},...
       {'MHHW (1.57m)location            '},...
       {'HAT (2.12m) location            '}]';
 %ztide=[-0.631 -.058 .218 .774 1.344 1.566 2.119 ];
Shead=uicontrol(fig,'style','text','position',[240 20 180 40],...
    'string','Rank Profiles by','foregroundcolor','b','backgroundcolor',bc);
Smenu=uicontrol(fig,'style','popup','position',[180 1 320 40],...
