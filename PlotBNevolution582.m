...
ndx=find(strcmpi({SM.Source},'ig8wheel')& [SM.Datenum] > 738448 & [SM.Datenum] < 738554);
...
xlabel(['MOP:' num2str(MopNumber) '  Cross-shore Distance (m)']);
...
%     if(nn > 8);set(p(nn-8),'color',[.9 .9 .9],'linewidth',1);end
    ...
    gif('BNevolution582.gif','overwrite',true,'DelayTime',.2)
   else
    gif('frame',gcf)
   end
    end
