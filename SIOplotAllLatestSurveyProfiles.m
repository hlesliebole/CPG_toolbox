...
m=0;
for MopNumber=507:514
m=m+1;
load(['M00' num2str(MopNumber,'%2.2i') 'SA.mat'])

n=2;%size(SA,2)-2;
%ndx=find([SA.Datenum] > datenum(2021,8,1));
...
set(gca,'ylim',[-.2 3.5],'xdir','reverse','fontsize',14);

title(['Surveys on ' datestr(SA(n).Datenum)],...
   'fontsize',16);

set(gca,'xlim',[-20 90]);xl=get(gca,'xlim');
plot(xl,[2.119 2.119],'k--');text(xl(1)+10,2.26,' HAT','fontsize',14);
plot(xl,[1.566 1.566],'k--');text(xl(1)+10,1.7,' MHHW','fontsize',14);
plot(xl,[1.344 1.344],'k--');text(xl(1)+10,1.44,' MHW','fontsize',14);
plot(xl,[.774 .774],'k--');text(xl(1)+10,.9,' MSL','fontsize',14);
plot(xl,[0.218 0.218],'k--');text(xl(1)+10,.34,' MLW','fontsize',14);
plot(xl,[-0.058 -0.058],'k--');text(xl(1)+10,0.05,' MLLW','fontsize',14);  
