...
% find desired Survey based on date and source
nsrv=size(Survey,2);
...
for m=1:size(N,2)
  if ~isempty(N(m).SA)
      fprintf('Saving Mop %i\n',m)
   if exist(['M' num2str(m,'%5.5i') 'SA.mat'],'file')
    ...
    save(['M' num2str(m,'%5.5i') 'SA.mat'],'SA');
    nsa=nsa+1;
  end
end
%fprintf('%i matfiles created.\n',nsa)
