function PlotLabelMopTransectUTM(MopID,PlotMethod,LineColor,ShadeOnOff)
...
% plot transect
 ...
 if contains(PlotMethod,'3d','IgnoreCase',true) % 3D plot
  ...
  if strcmpi(ShadeOnOff,'ShadeOn') && MopNumber > 2
    ...
    zlabel('NAVD88 (m)');
    hold on;
    text(Mop.OffXutm(n),Mop.OffYutm(n),zd,[num2str(MopNumber) '  '],...
...
% option to shade Mop area being viewed on 2D plot
  ...
  if strcmpi(ShadeOnOff,'ShadeOn') && MopNumber > 2
    ...
    text(Mop.BackXutm(n),Mop.BackYutm(n),['    ' num2str(MopNumber)],...
        'horizontalalign','left','fontsize',14,'fontweight','bold',...
        'color',LineColor)
 end
