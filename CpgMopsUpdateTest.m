%
%  Updates Mop SA matfiles in 4 ways:
%    - adds new surveys (struct array NewSurvey.File)
%    - replaces data for survey files that have been modified
%      (ModifiedSurvey.File)
%    - renames and surveys with changed file names
%      (RenamedSurvey.File  .NewFile)
%    - delete any surveys whos files no longer exist
%      (DeletedSurvey.File)
%
%   Modified, renamed and deleted surveys all have known Mop
%   SA files that include their data.  A list of all the Mop
%   unique numbers that have surveys in these 3 catagories is 
%   created under the variable MopsToEdit
...
% load the most recent check survey struct array as "CheckSurvey" struct aray
load('CheckSurvey.mat','Survey');
CheckSurvey=Survey;
CheckSurvey(1).NearestMops=[]; % add a nearest mops field to the struct array
...
% %  that are *not* in the CheckSurvey struct array
% 
% idx1=find(~ismember(lower({Survey.File}),lower({CheckSurvey.File})));
% 
% fprintf('-----------------------------------------------------\n')
% fprintf('There are %i filenames in the previous Survey list that are not in CheckSurvey.\n',numel(idx1))
% fprintf('-----------------------------------------------------\n')
% 
% if numel(idx1) > 0
%     for n=idx1
...
% Find filenames in the CheckSurvey struct array that
%  are not found in the Survey struct array 

idx2=find(~ismember(lower({CheckSurvey.File}),lower({Survey.File})));

fprintf('-----------------------------------------------------\n')
fprintf('There are %i new filenames in CheckSurvey that are not in Survey.\n',numel(idx2))
fprintf('-----------------------------------------------------\n')
...
if numel(idx2) > 0
    ...
    for n=idx2
        nn=nn+1;
     fprintf('%s\n',CheckSurvey(n).File)
     NewSurvey(nn).File=CheckSurvey(n).File;
    end
end
...
fprintf('-----------------------------------------------------\n')
fprintf('There are %i missing filenames in the previous Survey list that are not in CheckSurvey.\n',numel(idx1))
fprintf('-----------------------------------------------------\n')



fprintf('Checking for missing-old/new file survey date and bytesize match.\n')
fprintf('-----------------------------------------------------\n')
if numel(idx2) > 0
    nn=0;
    for n=idx2
        % find and combined match of survey date and file size
        idxmatch=find([Survey(idx1).Datenum] == CheckSurvey(n).Datenum & [Survey(idx1).Bytes] == CheckSurvey(n).Bytes);
        % if a match is found to a single file, replace the name in the
        % corresponding SA, SG and SM files
        if numel(idxmatch) == 1
            nn=nn+1;
           fprintf('\n%s\n new survey date, source, and file size matches:\n',CheckSurvey(n).File)
               fprintf('%s\n',Survey(idx1(idxmatch)).File)
               RenamedSurvey(nn).File=CheckSurvey(n).File;
               RenamedSurvey(nn).RenamedFile=Survey(idx1(idxmatch)).File;
%                fprintf('Updating the for SA, SG, SM filename for Mops:%s-%s\n',...
%                   num2str(min(Survey(idx1(idxmatch)).NearestMops)),...
%                   num2str(max(Survey(idx1(idxmatch)).NearestMops)))
%            for imop=Survey(idx1(idxmatch)).NearestMops
%               % update SA survey file name
%                matfile=[mpath 'M' num2str(imop,'%5.5i') 'SA.mat'];
%                load(matfile,'SA');
%                mdx=find(strcmp({SA.File},Survey(idx1(idxmatch)).File));
%                if numel(mdx) == 1
%                 %fprintf('%s match index: %i\n',matfile,mdx)
%                 SA(mdx).File=CheckSurvey(n).File; % update the matched SA file name
%                 save(matfile,'SA'); % save update struct array
%                end
%               % update SG survey file name
%                matfile=[mpath 'M' num2str(imop,'%5.5i') 'SG.mat'];
%                load(matfile,'SG');
%                mdx=find(strcmp({SG.File},Survey(idx1(idxmatch)).File));
%                if numel(mdx) == 1
%                 %fprintf('%s match index: %i\n',matfile,mdx)
%                 SG(mdx).File=CheckSurvey(n).File; % update the matched SG file name
%                 save(matfile,'SG'); % save update struct array
%                end
%               % update SM survey file name
%                matfile=[mpath 'M' num2str(imop,'%5.5i') 'SM.mat'];
%                load(matfile,'SM');
%                mdx=find(strcmp({SM.File},Survey(idx1(idxmatch)).File));
%                if numel(mdx) == 1
%                 %fprintf('%s match index: %i\n',matfile,mdx)
%                 SM(mdx).File=CheckSurvey(n).File; % update the matched SM file name
%                 save(matfile,'SM'); % save update struct array
%                end
%            end
           % finally, change file name in Survey struct array before
           %  the next comparison search for deleted files
           Survey(idx1(idxmatch)).File=CheckSurvey(n).File;
        end
    end
end

fprintf('Deleting SA, SG, SM entries for files that no longer exist. \n')
fprintf('------------------------------------------------------------\n')
if numel(idx1) > 0
    nn=0;
    for n=idx1
        
         fprintf('Removing: %s\n for SA, SG, SM Mops:%s-%s\n',Survey(n).File,...
     num2str(min(Survey(n).NearestMops)),num2str(max(Survey(n).NearestMops)))
    nn=nn+1;
    DeletedSurvey(nn).File=Survey(n).File;
%            % loop though mops that have data from this file
%            for imop=Survey(n).NearestMops
%               % update SA survey file name
%                matfile=[mpath 'M' num2str(imop,'%5.5i') 'SA.mat'];
%                load(matfile,'SA');
%                mdx=find(strcmp({SA.File},Survey(n).File));
%                if numel(mdx) == 1
%                 %fprintf('%s match index: %i\n',matfile,mdx)
%                 SA(mdx)=[]; % delete the SA entry
%                 save(matfile,'SA'); % save update struct array
%                end
%               % update SG survey file name
%                matfile=[mpath 'M' num2str(imop,'%5.5i') 'SG.mat'];
%                load(matfile,'SG');
%                mdx=find(strcmp({SG.File},Survey(n).File));
%                if numel(mdx) == 1
%                 %fprintf('%s match index: %i\n',matfile,mdx)
%                 SG(mdx)=[]; % delete the SG entry
%                 save(matfile,'SG'); % save update struct array
%                end
%               % update SM survey file name
%                matfile=[mpath 'M' num2str(imop,'%5.5i') 'SM.mat'];
%                load(matfile,'SM');
%                mdx=find(strcmp({SM.File},Survey(n).File));
%                if numel(mdx) == 1
%                 %fprintf('%s match index: %i\n',matfile,mdx)
%                 SM(mdx)=[]; % delete the SM entry
%                 save(matfile,'SM'); % save update struct array
%                end
%            end
    end
...
% Now delete the Survey struct array entries that are not in CheckSurvey
if numel(idx1) > 0
    fprintf('Removing the deleted entries from struct array Survey:\n')
    for n=fliplr(idx1)  % go from highest to lowest index to delete the right indices
     %fprintf('  %s\n',Survey(n).File)
     Survey(n)=[];
    end
end
...
if numel(idx2) > 0
    fprintf('Adding the following new surveys to Survey and the Mop mat files:\n')
    nn=size(Survey,2);
    mm=0;
    for n=idx2
      fprintf('%s\n',CheckSurvey(n).File)
      nn=nn+1;
      Survey(nn)=CheckSurvey(n);
      mm=mm+1;
      NewSurvey(mm).File=CheckSurvey(n).File;
     
%       [x,y,z,c,utmzone]=readSurveyFileUTM2(CheckSurvey(n).Source,CheckSurvey(n).File);
%    
%       if ~isempty(x)
...
nn=0;
for n=1:size(CheckSurvey,2)
    idx=find(strcmpi({Survey.File},CheckSurvey(n).File) == 1);
    if ~isempty(idx) 
        if Survey(idx).FileDatenum ~= CheckSurvey(n).FileDatenum
            
          fprintf('FileDatenum change; Updating Nearest Mop information for:\n%s\n',Survey(idx).File)
          nn=nn+1;
          ModifiedSurvey(nn).File=Survey(idx).File;
...
%           end
%    
%           Survey(idx).FileDatenum=CheckSurvey(n).FileDatenum; % update file datenum
...
% check to make sure group and drone folders on reefbreak are still mounted
%  before saving changes
    if exist('/volumes/group','dir')  && exist('/volumes/drone','dir')
    % updated saved Survey mater list struct array with nearest mop info
    fprintf('Saving struct array Survey with nearest Mops to SurveyMasterListWithMops.mat\n')
     save('SurveyMasterListWithMops.mat','Survey');
    fprintf('Saving updated Mops list to UpdatedMops.mat\n')
    UpdatedMops=unique(UpdatedMops); % get unique updated mop list
     save('UpdatedMops.mat','UpdatedMops');
     
%      % Update Mop SG and SM Files
%     fprintf('Updating corresponding SG and SM matfiles.\n')
...
for m=1:size(N,2)
  if ~isempty(N(m).SA)
   ...
   if exist([mpath 'M' num2str(m,'%5.5i') 'SA.mat'],'file')
    ...
    SA2=N(m).SA;
    SA=[SA2 SA]; % place new data first so not deleted if duplicate file found
    
    if size(SA,2) > 1
        
    %  Check again if there are any file names in the SA file that
    %  are not found in the Survey struct array 
