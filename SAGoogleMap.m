...
% overlay roughness points
zmin=0;zmax=0.5;set(gca,'clim',[0 .5]); % show 0-0.5 stdevroughness range
...
%-----------------------------------
% Plot mean depth values on google map
%-----------------------------------
z=vertcat(RC.Z); % use mean filter window depth as z
% only show below MSL
...
plot_google_map('MapType', 'satellite','Alpha', 1,'axis',ax1)
% overlay roughness points
zmin=-20;zmax=0.774;set(gca,'clim',[zmin zmax]); % show -20 to MSL range
zrange=zmax-zmin; % set max slope for coloring
zscaled = 1+64*(z-zmin)/zrange;
zscaled(zscaled < 1)=1;zscaled(zscaled > 64)=64;
idx=find(~isnan(zscaled)); % non NaN points 
...
set(ax1,'xlim',[min(x) max(x)],'ylim',[min(y) max(y)]);
cb=colorbar('horizontal');box on;
cb.Label.String='2D Spatial Mean Water Depth (m, NAVD88)';
xlabel('Longitude');ylabel('Latitude');
title([{'USACE SHOALS Topo-Bathymetric LiDAR Elevations'},...
    {'Using a 11m x 11m 2D Moving Filter'}]);

% %----------------------Plot Google Map Functions----------------------------
% 
