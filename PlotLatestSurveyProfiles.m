...
x0=find(SM(1).X1D == 0);
x10=find(SM(1).X1D == 10);
for n=1:size(SM,2)
    Zm10(n)=SM(n).Z1Dmean(x10);
    Zm(n)=SM(n).Z1Dmean(x0);
end
figure;plot(Zm,'*');
...
x=-14:30;
idx=find(ismember([SM(1).X1D],x));
eUTM=Mop.BackXutm(568)+x*cosd(90-Mop.Normal(568));
nUTM=Mop.BackYutm(568)+x*cosd(90-Mop.Normal(568));
z20211217=SM(409).Z1Dmean(idx);
z20221019=SM(456).Z1Dmean(idx);
z20230131=SM(473).Z1Dmean(idx);

save Mop568TypicalProfiles.mat x eUTM nUTM z20211217 z20221019 z20230131 


for m=[409 456]
%for m=[176 265 409 456 473]%n=1:nr    
    %m=size(SM,2)-nr+n;
...
xl=get(gca,'xlim');
set(gca,'xlim',[0 xl(2)]);
set(gca,'xlim',[-15 40]);xl(2)=40;
%yl=get(gca,'ylim');set(gca,'ylim',[0 yl(2)]);

plot(xl,[1.344 1.344],'k--');text(xl(2),1.45,' MHW','fontsize',14);
