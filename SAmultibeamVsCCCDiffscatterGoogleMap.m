...
MopStart=find(strcmp([Mop.Name],'D0860'));
MopEnd=find(strcmp([Mop.Name],'D0942'));
...
%%
cdx=find(strcmp({CS.Source},'CCC'));

x1=[CS(cdx(end-1)).X'   CS(cdx(end)).X' ]';
y1=[CS(cdx(end-1)).Y'   CS(cdx(end)).Y' ]';
z1=[CS(cdx(end-1)).Z'   CS(cdx(end)).Z' ]';
...
x2=vertcat(CS(idx(end)).X);
y2=vertcat(CS(idx(end)).Y);
z2=vertcat(CS(idx(end)).Z);
...
title({[datestr(CS(cdx(end)).Datenum) ' to ' datestr(CS(idx(1)).Datenum) ' |  Coastal Conservancy v Multibeam Change'],...
    ['Mean Change ' num2str(bias,'%5.3f') ' m.']},...
    'fontsize',16)
makepng(['Oceanside Multibeam' datestr(CS(idx(1)).Datenum,'YYYYmmDD') 'vsCCC' datestr(CS(cdx(end)).Datenum,'YYYYmmDD') 'Mops' num2str(MopStart) 'to' num2str(MopEnd) 'Change.png'])
...
hold on;
[ScatterPlot,ColorBarPlot]=ColorScatter2d(lon,lat,zdiff-bias);
hold on
load MopTableUTM.mat
for n=MopStart:MopEnd
plot([Mop.BackLon(n) Mop.OffLon(n)],[Mop.BackLat(n) Mop.OffLat(n)],'m-')
...
title({[datestr(CS(cdx(end)).Datenum) ' to ' datestr(CS(idx(1)).Datenum) ' | Coastal Conservancy v Multibeam Change'],...
    ...
    'fontsize',16)
makepng(['Oceanside Multibeam' datestr(CS(idx(1)).Datenum,'YYYYmmDD') 'vsCCC' datestr(CS(cdx(end)).Datenum,'YYYYmmDD') 'DebiasedChange.png'])
...
plot(z2,z1,'k.');hold on;plot([-35 0],[-35 0],'m-','linewidth',2)
grid on;set(gca,'xlim',[-35 0],'ylim',[-35 0],'fontsize',14);
xlabel([datestr(CS(idx(1)).Datenum) ' Multibeam (m, NAVD88)']);
ylabel([datestr(CS(cdx(end)).Datenum) ' Coastal Conservancy Depth (m, NAVD88)']);
title({'Oceanside Multibeam vs Coastal Conservancy :',['Comparison of N= ' num2str(N) ' overlapping 1m spatially-averaged depths']},'fontsize',16)
...
makepng('OceansideMultibeam6Feb2025VsCCC14Sep2011Scatterplot.png')
