...
Mop1=497; % marine room 
Mop2=682; % san elijo inlet
...
MopShoreline=table(MopNum,TrkX0,DateNum,Year,Month,Day,MhhwDist,MhwDist,MslDist);
...
[zmin,iminMHHW]=min(abs(Z1D-mhhw)');
iminMHHW(isnan(zmin))=NaN;
iminMHHW(zmin > dztol)=NaN;
fprintf('%i surveys have a valid %6.3f m contour location data.\n',numel(find(~isnan(iminMHHW))),mhhw)
...
%      Shoreline.MhwNorth=cy(i);
     if isnan(iminMHHW(i))
         Shoreline.MhhwDist=NaN;
     else
         Shoreline.MhhwDist=X1D(iminMHHW(i)); % distance relative to truck back beach
     end
...
save MopShorelineMhhwV2.mat MopShoreline
% 
% 
% 
% 
