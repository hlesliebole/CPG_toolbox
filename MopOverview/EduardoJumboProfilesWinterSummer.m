...
idx=find((month(Zdatetime) == 12 & year(Zdatetime) == syear-1) | (month(Zdatetime) == 1 & year(Zdatetime) == syear));
...
for n=idx   
    if (month(Zdatetime(n)) == 12 & year(Zdatetime(n)) == 2022) | (month(Zdatetime(n)) == 1 & year(Zdatetime(n)) == 2023)
...
title(['MOP ' num2str(MopNumber) '  Winter Surveys'])
...
ndx=find((month(Zdatetime) == 12 & year(Zdatetime) == syear-1) | (month(Zdatetime) == 1 & year(Zdatetime) == syear));
if numel(idx) > 0 & numel(ndx) > 0
idx=idx(ceil(numel(idx)/2));
ndx=ndx(ceil(numel(ndx)/2));
for n=idx   
...
set(gca,'xdir','reverse','fontsize',16);grid on;box on;
%legend('location','northoutside','numcolumns',4)
title('Winter to Fall Elevation Change vs Cross-shore Distance')
ylabel('Change (m)')
xlabel('Cross-shore Distance (m)');
...
%legend('location','northoutside','numcolumns',4)
title('Winter to Fall Elevation Change vs Mean Winter-Summer Depth')
ylabel('Change (m)')
xlabel('Mean Depth (m)');

exportgraphics(gcf,'TorreyWinterFallProfileChange.png','Resolution',150)
