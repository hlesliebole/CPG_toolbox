...
for s=ldx
    ...
    ishal=find(z1d <= VolumeElevCutoff & z1dmin >= -4);
    ideep=find(z1dmin < -4);
    ivdeep=find(z1dmin < -6);
    ...
    %     VolCutElev(ns)=sum((z1d(ivol)-VolumeElevCutoff).*A1D(ia1d(ivol)));
          VolMin(ns)=sum((z1d(ivol)-z1dmin(ivol)).*a1dgood(ivol));
...
plot(Sdate(idx),VolvDeep(idx)/100,'g.-')
...
std(VolDeep(idx)/100),...
mean(VolvDeep(idx)/100),...
std(VolvDeep(idx)/100));
% 
% %% --------------------------------------------------------------------
