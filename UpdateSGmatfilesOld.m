...
%for SurvNum=1:1
    ...
    % inner loop through nearby Mop surveys
      ...
      if ~isempty(N1)
       ...
       tidx=find(vertcat(N1.SA.Datenum) == SurveyDatenum,1,'last');
       if ~isempty(tidx)         
              Xr=[Xr' N1.SA(tidx).X']';
              Yr=[Yr' N1.SA(tidx).Y']';
              Zr=[Zr' N1.SA(tidx).Z']';
              Cr=[Cr' N1.SA(tidx).Class']';
       end
      end
      
      if ~isempty(N2)
       tidx=find(vertcat(N2.SA.Datenum) == SurveyDatenum,1,'last');
       if ~isempty(tidx)         
              Xr=[Xr' N2.SA(tidx).X']';
              Yr=[Yr' N2.SA(tidx).Y']';
              Zr=[Zr' N2.SA(tidx).Z']';
              Cr=[Cr' N2.SA(tidx).Class']';
       end
      end
      
      if ~isempty(N3)
       tidx=find(vertcat(N3.SA.Datenum) == SurveyDatenum,1,'last');
       if ~isempty(tidx)         
              Xr=[Xr' N3.SA(tidx).X']';
              Yr=[Yr' N3.SA(tidx).Y']';
              Zr=[Zr' N3.SA(tidx).Z']'; 
              Cr=[Cr' N3.SA(tidx).Class']';
       end 
      end
      
      if ~isempty(N4)
       tidx=find(vertcat(N4.SA.Datenum) == SurveyDatenum,1,'last');
       if ~isempty(tidx)         
              Xr=[Xr' N4.SA(tidx).X']';
              Yr=[Yr' N4.SA(tidx).Y']';
              Zr=[Zr' N4.SA(tidx).Z']';
              Cr=[Cr' N4.SA(tidx).Class']';
       end 
      end
...
% Grid the elevation survey using Delaunay tesselation 
   ...
   % Gridded survey exists, transfer into the new struct array
        
     SG(SurvNum).X=OG(old).X;
     SG(SurvNum).Y=OG(old).Y;
     SG(SurvNum).Z=OG(old).Z;
     SG(SurvNum).Class=OG(old).Class;
    end    
end
