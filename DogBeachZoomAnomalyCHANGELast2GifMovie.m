...
tl=text(-117.275,mean([32.9729   32.9787]),{'Time of Year','  Positive ','  Elevation','  Anomaly','CHANGE (m)'},'color','w','fontsize',16,'horizontalalign','left');
...
% set(gca,'xlim',[-117.2762 -117.2651]);
% set(gca,'ylim',[32.9705   32.9811]);
% dx=.11;set(gca,'position',[0.1300-dx    0.1100-dx*.9    0.7750*1.2    0.8150*1.2])
set(gca,'xlim',[-117.2762 -117.2651]);
set(gca,'ylim',[32.9729   32.9787]);
...
if nn == 2
    gif('SolanaDogBeachZoomAnomalyChangeLast4.gif','overwrite',true,'DelayTime',.5)%,'resolution',72,'nodither')
    nn=nn+1;
